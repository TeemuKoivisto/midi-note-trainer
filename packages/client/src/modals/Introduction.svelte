<script lang="ts">
  ...
  let { class: klass = '', params, hideModal = () => undefined }: Props = $props()
</script>

<article class={`${klass} w-full p-2 pb-4`}>
  <p>Greetings! 👋</p>
  <p>
  ...
  <h3>I/O</h3>
  <p>
    Under the <b>I/O Settings</b> you find controls to connect your MIDI keyboard which prompts for Web
    MIDI permissions. You can also fix your velocity incase your keyboard plays quietly (like mine).
    The default velocity without MIDI keyboard is 80.
  </p>
  <p>
    Under <b>Virtual piano</b> you can toggle the crude virtual piano incase you are using a tablet or
    phone. It's not that sophisticated and I'd advice you to use at least laptop keyboard if possible
  ...
  <p>
    With <b>Set Hotkeys</b> you can visualize and customize the mapping. I explored keyboard layouts
    in different locales and noticed that my layout (ISO Finland) is quite rare so I wanted to make
    sure the hotkeys would work for others too. I have however fixed the layout to 14-14-13-13 key
    ISO which has meant cutting keys from other layouts or leaving them blank. To detect the pressed
    keys I am using this rather curious "code" property in
  ...
  <h3>Scales</h3>
  <p>
    I went quite deep while coding this app to learn as much about music theory as possible 😵‍💫. This
    meant figuring out how to programmatically compute both the scales and chords without resolving
    to crude heuristics. Incase you find any silly mistakes, please report to the
    <a
  ...
  <p>
    ...
    most cases you use either all flats or sharps for the notes in a specific scale (eg C♯) but
    since sometimes a note is specified as either flat or sharp interval (minor third 3♭), I
    ultimately resolve to use its accidental. I'm not sure if this is always the correct way but it
    was the most programming-friendly.
  </p>
  <p>
  ...
  <p>
    ...
    >
    which represents either a <code>major/minor/augmented/diminished</code> chord. However, outside
    the 7 modes this falls apart quickly as there might not be any diatonically viable triads. I
    suppose you
    <i>could</i>
    just construct a triad without following diatonicy but I think that loses any nuance of the scale.
  </p>
  <p>
    How I approached this is somewhat unscientific; I iterate every scale note and then map all the
    other notes as its intervals. Then I use major-minor-diminished-augmented-7th-6th-suspended-5th
    order to find the best suited <a href="https://en.wikipedia.org/wiki/Trichord" target="_blank"
      >trichord</a
    >
  ...
  <p>
    ...
    those already mapped as major don't indicate if they can be extended diatonically further. For some
    notes I only found a major second or perfect fourth suggesting a <code>sus</code>
    chord but then there's no fifth which leaves basically no options left. In those cases, I add a fifth
    and call it a day but that makes the trichord non-diatonic so uhh... Any suggestions are welcome.
  </p>
  <p>
  ...
  <h3>Chords</h3>
  <p>
    Similar to scales, you can input <b>Key</b>, <b>Scale</b> as well as root <b>Note</b> and have them
    shown for every chord. Key and scale only changes the notation of the accidentals. You can also select/unselect
    these chords to be used with the Write/Play Chords games. There is some quick-use buttons to select
    basic major/minor, seventh and all chords.
  </p>
  <h3>Score</h3>
  <p>
    You can here set the played range of the keyboard notes and what's used in the games. The arrow
    buttons shift the start or end of the range by 1 octave. The scale and key determines the
    notation of the notes as well as the key signature.
  </p>
  <p>
    ...
    convention seems to be to use the closest major/minor approximate of the scale and then add the
    accidentals for the rest.
  </p>
  <p>
    Programmatically speaking, it's much easier to consider key signature's function only as a time
    and space saver to declutter the score from accidentals and it's how I've approached it.
    ...
    the key signature would only include Bb, Eb and then use accidentals for the rest. I could add 5
    flats to the signature and then naturalize the As but that starts to get convoluted. Only reason
  ...
  <p>
    ...
    the intention of adding a few more in the years to come. I hope they are quite self-explanatory
    as there's no instructions how to play them. <b>Guess Pitches</b> is relatively useless for those
    who have no chance of acquiring perfect pitch but it's fun to try once in a while.
  </p>
  <p>
  ...
  <p>
    As an end note, I hope this app will be helpful in teaching you music notation! I found multiple
